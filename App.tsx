...
const DashboardView: React.FC<{ appointments: Appointment[]; services: Service[]; clients: Client[]; costs: CostItem[]; defaultTab?: 'daily' | 'weekly_list' | 'monthly_list'; onRemovePayment: (app: Appointment) => void; onNoShow?: (app: Appointment) => void; onViewPet?: (pet: Pet, client: Client) => void, isSummaryOnly?: boolean }> = ({ appointments, services, clients, costs, defaultTab = 'daily', onRemovePayment, onNoShow, onViewPet, isSummaryOnly = false }) => {
    const [activeTab, setActiveTab] = useState<'daily' | 'weekly_list' | 'monthly_list'>(defaultTab);
                    ...
                    <section className="animate-fade-in text-left">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6">
                            ...
                            <div className="bg-blue-50/50 p-5 rounded-3xl border border-blue-100 flex flex-col justify-between h-32 relative overflow-hidden">
                                ...
                                <div className="z-10">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Total de Pets</p>
                                    <h3 className="text-3xl font-black text-gray-800 leading-none">{weeklyStats.totalPets}</h3>
                                </div>
                            </div>
                            ...
                            <div className="bg-orange-50/50 p-5 rounded-3xl border border-orange-100 flex flex-col justify-between h-32 relative overflow-hidden">
                                ...
                                <div className="z-10">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Total de Tosas</p>
                                    <div className="flex items-baseline gap-1">
                                        <h3 className="text-3xl font-black text-gray-800 leading-none">{weeklyStats.totalTosas}</h3>
                                        <span className="text-[9px] font-medium text-gray-400">Máquina e Tesoura</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-purple-50/50 p-5 rounded-3xl border border-purple-100 flex flex-col justify-between h-32 relative overflow-hidden">
                                ...
                                <div className="z-10">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">Tosa Higiênica</p>
                                    <h3 className="text-3xl font-black text-gray-800 leading-none">{weeklyStats.totalHygienic}</h3>
                                </div>
                            </div>
                        </div>
...
const ScheduleManager: React.FC<{ appointments: Appointment[]; clients: Client[]; services: Service[]; onAdd: (app: Appointment | Appointment[], client: Client, pet: Pet, services: Service[], duration: number) => void; onEdit: (app: Appointment, client: Client, pet: Pet, services: Service[], duration: number) => void; onUpdateStatus: (id: string, status: Appointment['status']) => void; onDelete: (id: string) => void; isOpen: boolean; onClose: () => void; onOpen: () => void; onLog: (a: string, d: string) => void; hidePrice?: boolean; }> = ({ appointments, clients, services, onAdd, onEdit, onUpdateStatus, onDelete, isOpen, onClose, onOpen, onLog, hidePrice }) => {
    ...
    const timeOptions: string[] = []; for (let h = 6; h <= 19; h++) { ['00', '10', '20', '30', '40', '50'].forEach(m => { if (h === 19 && m !== '00') return; timeOptions.push(`${String(h).padStart(2, '0')}:${m}`); }); }
    ...
    const renderWeekView = () => {
        ...
        return (
            <div className="flex h-full bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden flex-col mx-1">
                ...
                <div className="flex-1 overflow-y-auto relative flex"> <div className="w-10 bg-gray-50/30 border-r border-gray-100 flex-shrink-0 sticky left-0 z-10"> {Array.from({ length: 14 }, (_, i) => i + 6).map(h => (<div key={h} className="h-[120px] border-b border-gray-100 text-[9px] text-gray-400 font-bold p-1 text-right relative bg-gray-50/30"> <span className="-top-2 relative">{h}:00</span> </div>))} </div> {days.map(dIdx => {
...
const FinancialView: React.FC<{ appointments: Appointment[]; services: Service[]; clients: Client[]; costs: CostItem[]; defaultTab?: 'daily' | 'weekly' | 'weekly_list' | 'monthly' | 'monthly_list' | 'yearly'; onRemovePayment: (app: Appointment) => void; onNoShow?: (app: Appointment) => void; onViewPet?: (pet: Pet, client: Client) => void, isSummaryOnly?: boolean }> = ({ appointments, services, clients, costs, defaultTab = 'daily', onRemovePayment, onNoShow, onViewPet, isSummaryOnly = false }) => {
            ...
            {activeTab === 'daily' && (
                <section key={selectedDate} className={animationClass}>
                    <div className="sticky top-0 z-30 flex justify-between items-end mb-4 bg-white/90 backdrop-blur-md p-4 rounded-3xl border border-gray-100 shadow-sm transition-all">
                        ...
                        <div className="relative" onClick={() => (document.getElementById('daily-date-picker-fin') as HTMLInputElement)?.showPicker()}>
                            <button className="w-10 h-10 bg-gray-900 text-white rounded-2xl flex items-center justify-center shadow-lg hover:scale-105 active:scale-95 transition-all">
                                <CalendarIcon size={18} />
                            </button>
                            <input
                                id="daily-date-picker-fin"
                                type="date"
                                value={selectedDate}
                                onChange={(e) => { if (e.target.value) setSelectedDate(e.target.value); }}
                                className="absolute inset-0 opacity-0 cursor-pointer w-full h-full z-50 appearance-none"
                            ...
                            <div className="h-64 w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={monthlyChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                        ...
                                        <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#9ca3af', fontWeight: 600 }} dy={10} />
                                        <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#9ca3af' }} tickFormatter={(value) => `R$${value}`} />
                                        <Tooltip contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 30px -5px rgba(0, 0, 0, 0.1)' }} formatter={(value: number) => [`R$ ${value}`, 'Faturamento']} />
                                        <Area type="monotone" dataKey="faturamento" stroke="#8b5cf6" strokeWidth={3} fillOpacity={1} fill="url(#colorRevenue)" />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                ...
                activeTab === 'yearly' && metricData && (
                    <section className="animate-fade-in text-left">
                        <div className="sticky top-0 z-30 flex justify-between items-center mb-6 bg-white/90 backdrop-blur-md p-4 rounded-2xl border border-gray-100 shadow-sm">
                            ...
                            <select value={selectedYear} onChange={e => setSelectedYear(Number(e.target.value))} className="bg-gray-50 border-0 rounded-lg text-sm font-bold text-gray-700 outline-none focus:ring-2 ring-brand-100 p-1">
                        ...
                        <div className="bg-white rounded-[2.5rem] p-6 shadow-soft border border-gray-100/50 mb-8 overflow-hidden relative">
                            <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-radial from-brand-50 to-transparent opacity-50 pointer-events-none" />
                            <h3 className="text-sm font-bold text-gray-500 mb-6 uppercase tracking-wider flex items-center gap-2"><PieChartIcon size={16} /> Visão Anual</h3>
                            <div className="h-64 w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={yearlyChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }} barSize={12}>
                                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f3f4f6" />
                                        <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#9ca3af', fontWeight: 600 }} dy={10} />
                                        <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#9ca3af' }} tickFormatter={(value) => `R$${value}`} />
                                        <Tooltip cursor={{ fill: '#f9fafb' }} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 30px -5px rgba(0, 0, 0, 0.1)' }} formatter={(value: number) => [`R$ ${value}`, 'Faturamento']} />
                                        <Bar dataKey="faturamento" radius={[6, 6, 6, 6]}>
                                            {yearlyChartData.map((entry, index) => (
                                                <Cell key={`cell-year-${index}`} fill={entry.faturamento >= (yearlyChartData[index - 1]?.faturamento || 0) ? '#10b981' : '#3b82f6'} />
                                            ))}
                                        </Bar>
                                    </BarChart>
                                </ResponsiveContainer>
...
const App: React.FC = () => {
    ...
    return (
        <HashRouter>
            ...
            >
                {currentView === 'home' && <DashboardView appointments={appointments} services={services} clients={clients} costs={costs} defaultTab="daily" onRemovePayment={handleRemovePayment} onNoShow={handleNoShow} onViewPet={(pet, client) => setPetDetailsData({ pet, client })} isSummaryOnly={true} />}
                {currentView === 'revenue' && <FinancialView appointments={appointments} services={services} clients={clients} costs={costs} defaultTab="monthly" onRemovePayment={handleRemovePayment} onNoShow={handleNoShow} onViewPet={(pet, client) => setPetDetailsData({ pet, client })} isSummaryOnly={false} />}
